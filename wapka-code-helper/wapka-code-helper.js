...
types:{
 ...
 6:{n:'Online User',c:'Users',q:'Show currently active visitors with details.',e:'Lists visitors browsing the site. Filter by user or page.',p:{'ID, ID_NOT':'Set USERID=0 for guests only or ID_NOT=0 for logged users only','PAGEID, PAGEID_NOT':'Filter by page','PAGE':'Page number','ORDER':'NEW or OLD','LIMIT':'Items per page (max 30, default 20)','URL':'Required for pagination'},v:{'%userid%':'User ID','%username%':'Username','%date%':'Last active time','%browser%':'Browser name','%os%':'Device OS','%useragent%':'HTTP user agent','%avatar%':'Profile picture','%point%':'Credit points','%var(name)%':'Extra saved data','%pageid%':'Current page ID','%pagename%':'Page name','%location%':'Full URL (admin only)','%country%':'Country from IP','%ip%':'IP address','%role%':'User role','%count%':'Users shown','%pagenum%':'Total pages','%total%':'Total users','{{PAGING(%prev% %num% %next%)}}':'Pagination'}},
...
tags:{
  ...
  'PAGING':{n:'Pagination',d:'Generate pagination HTML.',t:'{{PAGING(%prev% %num% %next%)}}',g:'Control'}
},
...
function buildLuaTab(){
  ...
  for(var i=0;i<libs.length;i++){var l=libs[i];
    ...
    for(var j=0;j<l.i.length;j++)    h+='<div class="wkh-row"><span class="wkh-k">'+esc(l.i[j].n)+'</span><span class="wkh-v">'+esc(l.i[j].d)+'</span></div>';
    h+='</div>';
  }
